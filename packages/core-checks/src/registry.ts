 ...
 * server/governance/packageVerifier.ts (fetchNpm / fetchPypi) but without the
...
async function fetchNpm(pkg: string): Promise<RegistryResult> {
  ...
  if (res.status === 404) return { exists: false, isNewPackage: false, publishedAt: null };
  ...
  const firstVersionTime = data.time?.created ? new Date(data.time.created) : null;
...
function parseNpmTrustSignals(data: any): Pick<RegistryResult, "hasInstallScripts" | "hasProvenance" | "isDeprecated" | "hasRepository"> {
...
async function fetchPypi(pkg: string): Promise<RegistryResult> {
  ...
  const allDates = Object.values(releases)
    ...
    .filter(Boolean)
    .sort();
  const firstPublished = allDates.length ? new Date(allDates[0] as string) : null;
...
async function fetchCrates(pkg: string): Promise<RegistryResult> {
  ...
  const res = await fetch(url, { signal: AbortSignal.timeout(5000), headers: { "User-Agent": CRATES_UA } });
  if (res.status === 404) return { exists: false, isNewPackage: false, publishedAt: null };
  if (!res.ok) throw new Error(`crates.io HTTP ${res.status}`);

  const data = (await res.json()) as any;
  const created = data?.crate?.created_at ? new Date(data.crate.created_at) : null;
  const isNewPackage = created ? Date.now() - created.getTime() < NEW_PACKAGE_WINDOW_MS : false;
...
async function fetchGems(pkg: string): Promise<RegistryResult> {
  ...
  const dates = versions.map((v) => v?.created_at).filter(Boolean).sort();
  const first = dates.length ? new Date(dates[0] as string) : null;
  const isNewPackage = first ? Date.now() - first.getTime() < NEW_PACKAGE_WINDOW_MS : false;
...
async function fetchGoModule(pkg: string): Promise<RegistryResult> {
  ...
  if (res.status === 404 || res.status === 410) return { exists: false, isNewPackage: false, publishedAt: null };
...
const REGISTRY_CLIENTS: Record<Ecosystem, (pkg: string) => Promise<RegistryResult>> = {
  npm: fetchNpm,
  pypi: fetchPypi,
  crates: fetchCrates,
  gems: fetchGems,
 ...
 * number down. The design formula (§0) forbids exactly that: a figure on a
 * surface comes from the source the product reads from, never from a literal
 * typed into a design file. The landing's registry band is the first consumer.
 *
 ...
 */
export const ECOSYSTEMS: readonly Ecosystem[] = Object.keys(REGISTRY_CLIENTS) as Ecosystem[];
 ...
 */
export async function packageExists(pkg: string, ecosystem: Ecosystem): Promise<RegistryResult> {
  try {
    return await REGISTRY_CLIENTS[ecosystem](pkg);
  } catch {
    return { exists: true, isNewPackage: false, publishedAt: null, unreachable: true };
  }
}
