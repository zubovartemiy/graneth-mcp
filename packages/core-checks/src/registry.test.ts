...
describe("packageExists — npm", () => {
  ...
  it("200 with an old first publish → exists, not new", async () => {
    mockFetchOnce(200, {
      versions: { "1.0.0": {} },
      time: { created: "2018-01-01T00:00:00.000Z" },
    });
...
describe("packageExists — a hostile package name cannot reshape the registry request", () => {
  ...
  it("Packagist: odd characters in the name are percent-encoded, the vendor/name slash is kept", async () => {
    ...
    expect((await packageExists("evil@host/x?a=b#f", "composer")).exists).toBe(
      false
    );
  });
  ...
  it("Packagist: a real vendor/name is passed through untouched", async () => {
    const fetchMock = vi.fn(async (url: string) => {
      ...
      return {
        ok: true,
        status: 200,
        json: async () => ({ package: { time: "2011-01-01T00:00:00+00:00" } }),
      } as any;
    });
    vi.stubGlobal("fetch", fetchMock);
    expect((await packageExists("monolog/monolog", "composer")).exists).toBe(
      true
    );
  });
  ...
  it("Go proxy: odd characters are encoded while the !-escaping and path slashes survive", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    expect((await packageExists("github.com/Azure/x y", "go")).exists).toBe(
      false
    );
  });
});
...
describe("packageExists — crates.io", () => {
  it("404 → does not exist", async () => {
    mockFetchOnce(404);
    expect((await packageExists("no-such-crate-xyz", "crates")).exists).toBe(
      false
    );
  });
  ...
  it("200 → exists; crate.created_at drives isNewPackage; sends a User-Agent (crates.io 403s without one)", async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, init: any) => {
      ...
      return {
        ok: true,
        status: 200,
        json: async () => ({ crate: { created_at: created } }),
      } as any;
    });
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("outage → fail-open unreachable", async () => {
    mockFetchOnce(500);
    expect(await packageExists("any-crate", "crates")).toMatchObject({
      exists: true,
      unreachable: true,
    });
  });
...
describe("packageExists — RubyGems", () => {
  ...
  it("outage → fail-open unreachable", async () => {
    ...
    expect(await packageExists("any-gem", "gems")).toMatchObject({
      exists: true,
      unreachable: true,
    });
  });
...
describe("packageExists — Packagist (composer)", () => {
  it("404 → does not exist", async () => {
    mockFetchOnce(404);
    expect((await packageExists("no-vendor/no-pkg", "composer")).exists).toBe(
      false
    );
  });
...
describe("packageExists — Go module proxy", () => {
  it("404 → does not exist", async () => {
    mockFetchOnce(404);
    expect((await packageExists("github.com/nope/nope", "go")).exists).toBe(
      false
    );
  });
  ...
  it("410 → does not exist (proxy uses 410 Gone for known-missing modules)", async () => {
    mockFetchOnce(410);
    expect((await packageExists("github.com/gone/gone", "go")).exists).toBe(
      false
    );
  });
...
describe("ECOSYSTEMS — the runtime set, so nothing downstream has to write a number", () => {
  it("names every registry the detector has a client for", () => {
    ...
    expect([...ECOSYSTEMS].sort()).toEqual([
      "composer",
      "crates",
      "gems",
      "go",
  ...
  it("is derived from the client table, not written beside it", async () => {
    ...
    for (const eco of ECOSYSTEMS) {
      mockFetchOnce(404);
      await expect(
        packageExists("definitely-not-a-real-package-xyz", eco)
