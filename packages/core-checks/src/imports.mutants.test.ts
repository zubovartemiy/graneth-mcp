...
describe("primaryEcosystem", () => {
  it("routes python and JS/TS extensions case-insensitively, null otherwise", () => {
    ...
    for (const ext of ["js", "jsx", "ts", "tsx", "mjs", "cjs", "mts", "cts"])
      expect(primaryEcosystem(`a.${ext}`)).toBe("npm");
...
describe("JS/TS extraction forms", () => {
  it("static ESM with and without bindings", () => {
    expect(refs("a.ts", `import axios from "axios";`)[0]).toMatchObject({
      pkg: "axios",
      line: 1,
    });
    expect(refs("a.ts", `import "polyfill-lib";`)[0]).toMatchObject({
      pkg: "polyfill-lib",
    });
    expect(refs("a.ts", `import { x, y } from 'some-lib'`)[0]).toMatchObject({
      pkg: "some-lib",
    });
  });
  ...
  it("dynamic import() incl. await and spacing", () => {
    expect(
      refs("a.ts", `const m = await import( 'lazy-lib' );`)[0]
    ).toMatchObject({ pkg: "lazy-lib" });
    // Space BETWEEN the keyword and the paren — kills the \s*→\S* regex mutants.
    expect(
      refs("a.ts", `const m = await import ('spaced-dyn-lib');`)[0]
    ).toMatchObject({ pkg: "spaced-dyn-lib" });
    expect(
      refs("a.cjs", `const x = require ('spaced-cjs-lib');`)[0]
    ).toMatchObject({ pkg: "spaced-cjs-lib" });
  });

  it("re-exports (barrel files)", () => {
    expect(
      refs("a.ts", `export { thing } from "barrel-lib";`)[0]
    ).toMatchObject({ pkg: "barrel-lib" });
    expect(refs("a.ts", `export * from "star-lib";`)[0]).toMatchObject({
      pkg: "star-lib",
    });
  });

  it("CommonJS require", () => {
    expect(refs("a.cjs", `const x = require("cjs-lib");`)[0]).toMatchObject({
      pkg: "cjs-lib",
    });
    expect(refs("a.cjs", `require( 'spaced-lib' )`)[0]).toMatchObject({
      pkg: "spaced-lib",
    });
  });

  it("roots deep paths and keeps the @scope/name pair intact", () => {
    expect(refs("a.ts", `import x from "lodash/merge";`)[0]).toMatchObject({
      pkg: "lodash",
    });
    expect(
      refs("a.ts", `import y from "@scope/pkg/deep/path";`)[0]
    ).toMatchObject({ pkg: "@scope/pkg" });
  });
  ...
  it("ignores node: protocol imports and bare Node builtins", () => {
    ...
    expect(
      refs("a.mjs", `import { spawn } from "node:child_process";`)
    ).toEqual([]);
    expect(refs("a.cjs", `const path = require("node:path");`)).toEqual([]);
    expect(
      refs("a.ts", `import fs from "fs";\nimport path from "path/posix";`)
    ).toEqual([]);
  });
  ...
  it("still extracts real scoped packages", () => {
    expect(refs("a.ts", `import { z } from "@scope/pkg";`)[0]).toMatchObject({
      pkg: "@scope/pkg",
    });
  });
});
...
describe("python extraction forms", () => {
  it("import / from-import at line start, first segment only", () => {
    expect(refs("a.py", `import numpy`)[0]).toMatchObject({
      pkg: "numpy",
      ecosystem: "pypi",
    });
    expect(refs("a.py", `from flask import request`)[0]).toMatchObject({
      pkg: "flask",
    });
    expect(refs("a.py", `import numpy.linalg`)[0]).toMatchObject({
      pkg: "numpy",
    });
  });

  it("normalizes underscores to dashes (registry naming)", () => {
    expect(refs("a.py", `import python_dateutil`)[0]).toMatchObject({
      pkg: "python-dateutil",
    });
  });
   ...
   *
   * It read "does not match indented (nested) imports or non-import lines" and
   * pinned `    import os_inside_function` → `[]` as intended behaviour. It is
   * not: a lazy import inside a function, a `try: import x / except
   * ImportError` fallback and an `if TYPE_CHECKING:` block are ordinary Python
   * and are precisely what a model writes for an optional dependency — and
   * `pyproject.toml` is deliberately not a manifest here, so for many Python
   * projects the import scan is the only place the name could be seen at all.
   * Rust and Go always allowed leading whitespace; Python alone did not.
   *
   * The half that WAS right is the second assertion, and it is the only reason
   * the anchor existed: a name inside a string literal is not a dependency.
   * That still holds, because `^\s*` allows indentation and nothing else.
   */
  it("matches an indented import, and still refuses one inside a string", () => {
    expect(refs("a.py", `    import os_inside_function`)[0]).toMatchObject({
      pkg: "os-inside-function",
    });
...
describe("non-ecosystem files", () => {
  it("files outside npm/pypi extensions yield no refs even with import-looking lines", () => {
    expect(
      refs("notes.txt", `import x from "some-lib";\nimport requests`)
    ).toEqual([]);
  });
});
...
describe("dedup and location", () => {
  it("keeps the first-seen location per (pkg, ecosystem)", () => {
    const out = extractImportedPackages([
      {
        path: "a.ts",
        content: `import a from "dupe-lib";\nimport b from "dupe-lib";`,
      },
  ...
  it("the same name counts once per ecosystem", () => {
    ...
    expect(new Set(out.map(r => r.ecosystem))).toEqual(
      new Set(["npm", "pypi"])
    );
  });
});
